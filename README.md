# aprendendohtml
PRIMEIRO PROJETO EM HTML
<!DOCTYPE html>
<html lang="pt-br">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>explicando o HTML</title>

</head>
<body>
    <body bgcolor="#A9A9A9" >
    <h1 id="inicio"> 
        
        <center>
            <font color ="8B0000">
            INTRODUÇÃO AO HTML
        </font>
        </center>
    </h1>
    <hr>
    <h2>  
         
     Neste blog você terá uma breve introdução sobre o <u>HTML</u>, compreendendo os elementos e caracteristicas de uma pagina na web.
    Para isso, siga a sequência a seguir:

</h2>

    <P>
       
        <font size="5" color= #FFFAFA >
        <ul>
           
            <li><a href="#o-que-é">o que é o HTML</a> </li>
            <li><a href="#o que são tags">o que são tags </a> </li>
            <li><a href="#Atributos"> atributos das Tags</a></li>
            <li><a href="#tipos">tipos de tags</a></li>
            
        </ul>
    </font>
    </P>
    <P>
        <h2 id= "o-que-é"> <CENTER> 1- O QUE É O HTML</CENTER></h2> </P>

        <small><a href="#inicio"><center>(voltar)</center></a></small><bR> </br>

    <img src="data:image/png;base64,"
 border ="2" align="center"hspace=390 >    <br>
</br>
<p>

 <p> <font size="4">
    Sigla para <strong>HyperText Markup Language</strong> — Linguagem de Marcação de Hipertexto  que serve na construção de  sites na web. Através dela 
    o usuario consegue criar toda a estrutura de um site,projetando o seu formato, conteúdo
    e personalizando a pagina, junto com o CSS.
</font>
 </p>
 <br>
</br>
    <h2 id="o que são tags"> <CENTER>2- O QUE SÃO TAGS</CENTER> </h2>
    <img src="https://www.homehost.com.br/blog/wp-content/uploads/2019/07/tag_html.png"
    border ="2" align="center"width=950 height=200 hspace=10 >

    <small><a href="#inicio"><center>(voltar)</center></a></small><bR> </br>
    <p>
        <font size="4">TAGs são os comandos utilizados pela linguagem HTML. <br>
        Cada TAG informa ao programa visualizador ou browser, como ele deverá formatar o texto e deve estar dentro dos sinais de menor que (<) e maior que (>).<br>
        Os TAGs podem ser únicos ou duplos, com início e fim.
        Algumas TAGS admitem de atributos (parâmetros) que alteram a maneira como o navegador deve interpretá-la e normalmente são colocados na TAG inicializadora.  
    </font>
    </p>
    <img src="https://miro.medium.com/v2/resize:fit:1400/0*eU6H-gxRpslRHfNT.jpg"
    border ="2" width=500 height=500  align= "center" hspace=250  >    <br>
</br>
    <h2 id="Atributos"> <center>3 - ATRIBUTOS DAS TAGS </center></h2>
    <P>   Atributos das Tags são caracteristicas que podem ser colocadas sobre as tags
        como personalização de <mark>fonte,cor,alinhamento,tamanho etc.</mark> 
        
        É possivel colocar essas caracteristicas desde que você entenda a estrutura da tag e a referência
        necessária da caracteristica que deseja.<br>
        <strong>por exemplo:</strong> colocamos a <u>< center></u> para centralizar a tag
    </P>   
    <small><a href="#inicio"><center>(voltar)</center></a></small><bR> </br> 
    <h2 id="tipos"> <CENTER>4- TIPOS DE TAG</CENTER> </h2>
    <P>
        <img src="https://lh6.googleusercontent.com/-0hJaxSCDxtY/TYvsTPXK6nI/AAAAAAAAABU/UOxuL4DMQFM/s1600/html5%255B1%255D.jpg"
    border ="2" width=500 height=500  align= "center" hspace=250  >
    </P>
    <p>
        
        Cada tag atribui uma função para a estrtura do seu blog, você precisa entender sua aplicação, o modo de aplicar
        e principalmente o efeito especifico no seu blog. se atender suas necessidades então você
        estará no caminho certo para formata-la em uma visualização mais adequada graças ao CSS.
    </p>
    <small><a href="#inicio"><center>(voltar)</center></a></small><bR> </br>

 
   
    

    
</body>
</html>
